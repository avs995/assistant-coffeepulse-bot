...
def _parse_quick_payload(payload: str) -> dict[str, str]:
    ...
    for line in normalized.splitlines():
        ...
        field = FIELD_MAP.get(key_norm)
        if field:
            parsed[field] = value_norm
    return parsed
...
async def cmd_quick(message: Message) -> None:
    ...
    score = _normalize_score(parsed.get("score", "medium")) if parsed.get("score") else "medium"
    status = _normalize_status(parsed.get("status", "new")) if parsed.get("status") else "new"
    if score is None:
